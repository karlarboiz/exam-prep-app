...
@WebServlet("/user/weekly")
public class WeeklyExamServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            ExamAttempt attempt = weeklyRegimenService.startWeeklyExam(user.getId());
            if (attempt.getStatus() != AttemptStatus.IN_PROGRESS) {
                resp.sendRedirect(req.getContextPath() + "/user/study-plan?regimenId="
                        + IdCipher.enc(attempt.getRegimenId()));
                return;
            }
            resp.sendRedirect(req.getContextPath() + "/user/weekly?attemptId=" + IdCipher.enc(attempt.getId()));
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (!attempt.getUserId().equals(user.getId()) || attempt.getAttemptKind() != AttemptKind.WEEKLY) {
            ...
            if ("submit".equals(action)) {
                ExamAttempt completed = weeklyRegimenService.submitWeeklyExam(attemptId, collectAnswers(req, attemptId));
                resp.sendRedirect(req.getContextPath() + "/user/study-plan?regimenId="
                        + IdCipher.enc(completed.getRegimenId()));
                return;
            }
    ...
    private void showExamPage(Long attemptId, User user, HttpServletRequest req, HttpServletResponse resp)
        ...
        if (weeklyRegimenService.isExpired(attempt)) {
            ExamAttempt completed = weeklyRegimenService.submitWeeklyExam(
        ...
        req.setAttribute("examPostPath", "/user/weekly");
