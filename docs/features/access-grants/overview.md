...
1. Funnel completes a purchase and calls `POST /api/access-tokens` with the shared API key.
2. This app generates a high-entropy raw token, stores only its SHA-256 hash (`UNUSED`), and returns the raw token once.
3. Buyer opens `/register?token=...`, creates an account.
